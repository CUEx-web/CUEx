...
function ProductBody(props) {
  ...
  /*if the user is an Admin or the owner of the product, he can see the edit and delete button*/
  ...
  function deleteproduct() {
    ...
    if (answer) {
      ...
      fetch(url, { method: method, credentials: "include" }).then((res) => {
        if (res.status !== 200) {
          //If there is any error, statusCode will not be 200 and will throw error
  ...
  // This set the product information
  useEffect(() => {
    ...
    fetch("http://localhost:3001/" + product.productPicture).then((resData) =>
      console.log(resData)
    );
  }, [product]);
  ...
  /*This is a fetch method to get the product owner Information*/
  ...
  /*Product will be Edited after clicking the submit form. */
  const submitted = (e) => {
    e.preventDefault();
    ...
    <div className="postpage" stlye={{ width: "100%" }}>
      ...
      <div className="product_body">
        <div className="body_image">
          <img
            src={"http://localhost:3001/" + product.productPicture}
            alt="user picture"
            id="product_img"
          />
        </div>
        <div className="body_about">
          ...
          <p>
            <div className="priceHeading">Price: ${product.price}</div>
          </p>
          <p>
          ...
          <p>
            Product description: <var>{product.description}</var>
          </p>
          <a href={email}>
      ...
      <div className="sellerinfo ">
        <div className="Seller_text ">
          <p>
            Grade: <var>{user.grade}</var>
          </p>
          <p>
            Email: <var>{user.email}</var>
            <br />
